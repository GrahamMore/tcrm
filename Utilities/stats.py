...
def getCellNum(lon, lat, gridLimit, gridSpace):
    ...
    j = int(abs((abs(lon) - abs(gridLimit['xMin']))) / abs(gridSpace['x']))
    i = int(abs((abs(lat) - abs(gridLimit['yMax']))) / abs(gridSpace['y']))
...
def getCellLonLat(cellNum, gridLimit, gridSpace):
    ...
    indLat = int(cellNum / lon.size)
    indLon = int(cellNum % lon.size)
